...
# Configuration
SITEMAP_DB = "products.db"  # Contains URLs with last_modified in ISO format
SCRAPED_DB = "scraped_data.db"  # Contains product data with SQL format datetimes
...
def get_existing_product(url):
    """Check if product exists and return its current price"""
    with sqlite3.connect(SCRAPED_DB) as conn:
        cursor = conn.cursor()
...
async def fetch_product(session, url):
    """Scrape individual product page with enhanced price handling"""
    try:
        # First check if product exists in database
        existing_price = get_existing_product(url)
        ...
        async with session.get(url, headers=HEADERS, timeout=30) as response:
            if response.status == 200:
                ...
                if out_of_stock:
                    if existing_price:
                        # Existing product now out of stock - keep old price
                    ...
                    # Product in stock - get current price
                ...
                print(f"✅ Updated: {title[:50]}... (Price: {price})")
